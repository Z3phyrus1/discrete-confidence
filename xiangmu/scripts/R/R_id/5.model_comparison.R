...
# ------------------------------------------------------------------ #
# load 
dfit <- read.table("D:/xiangmu/data/behavior/idsess/fit_par.txt",header=T)#D:/reysy/EEG/data/fit_par.txt
...
pdf("D:/xiangmu/figures/behavior_id/conf_fixb.pdf",width=1.6, height=1.6)
...
pdf("D:/xiangmu/figures/behavior_id/conf_bayes.pdf",width=1.6, height=1.6)
...
pdf("D:/xiangmu/figures/behavior_id/conf_discrete.pdf",width=1.6, height=1.6)
...
pdf("D:/xiangmu/figures/behavior_id/model_comp.pdf",height=4.8,width=1.3)
...
pdf("D:/xiangmu/figures/behavior_id/model_freq.pdf",height=2,width=1.3)
...
# ------------------------------------------------------#
# additional plot individual AIC values
...
p0 <- ggplot(d2,aes(x=reorder(id,AIC_diff),y=AIC_diff,fill=col_d))+geom_bar(stat="identity")+scale_fill_manual(values=c(sub_2_col,bayes_col),guide=F)+nice_theme+scale_x_discrete(labels=NULL,name="Participant")+labs(y="AIC difference")+coord_cartesian(ylim=c(-ceiling(max(d2$AIC_diff))/2.5,ceiling(max(d2$AIC_diff))))+annotate("text",x=3,hjust="left",y=max(d2$AIC_diff)*0.9,label="discrete better",color=sub_2_col,size=2.3)+annotate("text",x=3,hjust="left",y=-max(d2$AIC_diff)/5,label="Bayesian better",color=bayes_col,size=2.3)


d2 <- d_plot_i[which(d_plot_i$model=="biased-Bayesian"),]
d2$col_d <- ifelse(d2$AIC_diff>=0,sub_2_col,meta_col)
p1 <- ggplot(d2,aes(x=reorder(id,AIC_diff),y=AIC_diff,fill=col_d))+geom_bar(stat="identity")+scale_fill_manual(values=c(sub_2_col,meta_col),guide=F)+nice_theme+scale_x_discrete(labels=NULL,name=" ")+labs(y=" ")+coord_cartesian(ylim=c(-ceiling(max(d2$AIC_diff))/2.5,ceiling(max(d2$AIC_diff))))+annotate("text",x=3,hjust="left",y=max(d2$AIC_diff)*0.9,label="discrete better",color=sub_2_col,size=2.3)+annotate("text",x=3,hjust="left",y=-max(d2$AIC_diff)/5,label="biased-Bayesian better",color=meta_col,size=2.3)


d2 <- d_plot_i[which(d_plot_i$model=="fixed bias"),]
d2$col_d <- ifelse(d2$AIC_diff>=0,sub_2_col,meta_col)
p2 <- ggplot(d2,aes(x=reorder(id,AIC_diff),y=AIC_diff,fill=col_d))+geom_bar(stat="identity")+scale_fill_manual(values=c(sub_2_col,"dark grey"),guide=F)+nice_theme+scale_x_discrete(labels=NULL,name=" ")+labs(y=" ")+coord_cartesian(ylim=c(-ceiling(max(d2$AIC_diff))/2.5,ceiling(max(d2$AIC_diff))))+annotate("text",x=3,hjust="left",y=max(d2$AIC_diff)*0.9,label="discrete better",color=sub_2_col,size=2.3)+annotate("text",x=3,hjust="left",y=-max(d2$AIC_diff)/5,label="fixed bias better",color="dark grey",size=2.3)

pdf("D:/xiangmu/figures/behavior_id/AIC_iindividual.pdf",height=2.5,width=6.67)
