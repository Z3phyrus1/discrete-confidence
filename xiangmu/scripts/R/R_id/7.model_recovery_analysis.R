...
dfit <- read.table("D:/xiangmu/data/behavior/id/fit_par.txt",header=T)
...
# }
for(i in unique(dfit$id)){
  idx <- which.min(dfit[dfit$id==i,]$AIC)
  dfit$best_m[dfit$id==i] <- as.character(dfit[dfit$id==i,]$model[idx])
}
...
for(id in 1:n_id){
  ...
  save(sim_data, sim_fit, file="D:/xiangmu/data/behavior/id/model_recovery_res.RData")
...
# ------------------------------------------------------#
load("D:/xiangmu/data/behavior/id/model_recovery_res.RData")
...
ggsave(plot=pl1, filename="D:/xiangmu/figures/behavior_id/model_recovery.pdf", device="pdf",width=7.5,height=1.7)
