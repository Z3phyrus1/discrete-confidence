...
d <- read.table(file="D:/xiangmu/data/behavior/id/data_wide_wmPred.txt", sep=";", header=T)#D:/reysy/EEG/data/data_wide_wmPred.txt
...
dir.create("D:/xiangmu/figureures/behavior/id", recursive = TRUE)
pdf("D:/xiangmu/figureures/behavior_id/rt_01_430.pdf",width=2,height=2.4)

dev.off()

## do the same but for control conditions
# load data
d_c <- read.table("D:/xiangmu/data/behavior/id/data_nu.txt",header=T, sep=";")#D:/reysy/EEG/data/data_nu.txt
...
d$logrt2 <- log(d$rt2)
d$logrt1 <- log(d$rt1)
write.table(d, "D:/xiangmu/data/behavior/id/all_RTanalysis.txt",row.names=F, sep="/t")#D:/reysy/EEG/data/all_RTanalysis.txt
...
pdf("D:/xiangmu/figures/behavior_id/RT_resid.pdf",width=3.36,height=2.4)
