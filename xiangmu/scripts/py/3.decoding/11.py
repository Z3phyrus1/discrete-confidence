...
if __name__ == "__main__":  
    ...
    def run_sliding_analysis(X, y, times, task_name, label_name):
        ...
        T_obs_tfce, _, H0 = mne.stats.permutation_cluster_1samp_test(
            scores - 0.5,
            tail=1,
            n_permutations=1000,
        ...
        p_values = (np.sum(H0 >= T_obs_tfce[:, None], axis=1) + 1) / (H0.size + 1)

        # 显著掩码（单侧，p<0.05）
        sig_mask = p_values < 0.05

        # 伪造 clusters 结构，满足 plot_with_cluster_highlight 的接口期望
        clusters = [sig_mask] if sig_mask.any() else []
        cluster_p_values = np.array([p_values[sig_mask].min()]) if sig_mask.any() else np.array([])
        ...
        utils.plot_with_cluster_highlight(
            ...
            permutation_result=(clusters, cluster_p_values, H0),  # 传入伪造的 clusters
